setwd("/home/pgeeleher/postdoc_stuff/HDAC_project/Scripts/r_package_files/pRRophetic/vignettes/")

## load the library
library(pRRophetic)
sapply(paste("/home/pgeeleher/postdoc_stuff/HDAC_project/Scripts/r_package_files/pRRophetic/R/", dir("../R/"), sep=""), source)
library(car)
library(sva)
library(ridge)
library(ggplot2)
...
data("bortezomibData.RData") # load("/home/pgeeleher/postdoc_stuff/HDAC_project/Scripts/r_package_files/pRRophetic/data/bortezomibData.RData") # exprDataBortezomib, bortIndex, studyResponse, studyIndex,
...
response <- factor(c(rep("NR", sum((studyResponse == "PGx_Responder = NR") & bortIndex)), rep("R", sum((studyResponse == "PGx_Responder = R") & bortIndex))))
df <- data.frame(Pred=c(predictedPtype_blood[((studyResponse == "PGx_Responder = NR") & bortIndex)], predictedPtype_blood[((studyResponse == "PGx_Responder = R") & bortIndex)]), Response=response)
ggplot(data=df, aes(y=Pred, x=Response, fill=Response)) + geom_boxplot(alpha=.3) + theme_bw() + ylab("Predicted Bortezomib Sensitivity") + geom_hline(yintercept=cutpoint)

# Calculate positve and negative predictive values
...
nrs <- which(((studyResponse == "PGx_Responder = NR") & bortIndex))
rs <- which((studyResponse == "PGx_Responder = R") & bortIndex)

...
calcPhenotype <- function(trainingExprData, trainingPtype, testExprData, batchCorrect="eb", powerTransformPhenotype=TRUE, removeLowVaryingGenes=.2, minNumSamples=10, selection=-1, printOutput=TRUE, removeLowVaringGenesFrom="homogenizeData")
{
 ...
 is_matrix_safe <- function(x) {
 return("matrix" %in% class(x))
 }

 # Function to check if object is numeric (R 4.0+ compatible)
 is_numeric_safe <- function(x) {
 return("numeric" %in% class(x) || is.numeric(x))
 }

 # check if the supplied data are of the correct classes
 if (!is_matrix_safe(testExprData))
   stop("ERROR: \"testExprData\" must be a matrix.")
 if (!is_matrix_safe(trainingExprData))
   stop("ERROR: \"trainingExprData\" must be a matrix.")
 if (!is_numeric_safe(trainingPtype))
 ...
 # FIXED: Check if homData$test is a vector or single column
 if(is.vector(homData$test) || (!is.matrix(homData$test) && !is.data.frame(homData$test)) || ncol(homData$test) == 1)
 {
